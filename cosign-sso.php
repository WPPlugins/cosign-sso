...
Description: Alternative authentication plugin for WordPress. This plugin add several login method: LDAP login and CoSign Single Sign-on(SSO) v2, v3 login.
Version: 0.3.1
...
define('COSIGN_SSO_VERSION', "0.3");
define('COSIGN_SSO_FALLBACK_FILE', dirname(__FILE__) . '/FALLBACK');
define('SSO_LOGIN_DISABLED', 0);
define('SSO_LOGIN_LDAP',     1);
define('SSO_LOGIN_COSIGN_V2',      2);
define('SSO_LOGIN_COSIGN_V3',      3);
define('SSO_LOGIN_NO_FALLBACK',  999);
...
if( file_exists(COSIGN_SSO_FALLBACK_FILE)) {
	$fallback_content = trim(strtolower(file_get_contents(COSIGN_SSO_FALLBACK_FILE)));
	if ( $fallback_content == 'ldap' )
		$cosign_sso_fallback = SSO_LOGIN_LDAP;
	elseif ( $fallback_content == 'cosign2' )
		$cosign_sso_fallback = SSO_LOGIN_COSIGN_V2;
	elseif ( $fallback_content == 'cosign3' )
		$cosign_sso_fallback = SSO_LOGIN_COSIGN_V3;
	else
		$cosign_sso_fallback = SSO_LOGIN_DISABLED;
} else {
	$cosign_sso_fallback = SSO_LOGIN_NO_FALLBACK;
}
...
function cosign_real_login_method()
{
	global $cosign_sso_opt, $cosign_sso_fallback;

	if ($cosign_sso_fallback != SSO_LOGIN_NO_FALLBACK)
	{
		return $cosign_sso_fallback;
	}

	// back compatible with v 0.3.0
	if (isset($cosign_sso_opt["sso_protocol"]) && $cosign_sso_opt["sso_protocol"] == "3")
	{
		if ( (int)$cosign_sso_opt["login_method"] == SSO_LOGIN_COSIGN_V2 )
			$cosign_sso_opt["login_method"] = SSO_LOGIN_COSIGN_V3;
	}

	return (int)$cosign_sso_opt["login_method"];
}
...
function cosign_sso_activate()
{
	...
	$optionarray_def = array(
		'login_method' => SSO_LOGIN_DISABLED,
...
function cosign_sso_login_redirect()
{
	...
	if ( cosign_real_login_method() == SSO_LOGIN_COSIGN_V3 )
	{
		## CoSign V3: redirect to weblogin only with cookie name.
	...
	elseif ( cosign_real_login_method() == SSO_LOGIN_COSIGN_V2 )
	{
		## CoSign V2: set cookie first, then redirect to weblogin with
...
function cosign_sso_authenticate($user, $username="", $password="")
{
	...
	if ( in_array( cosign_real_login_method(), array(SSO_LOGIN_COSIGN_V2, SSO_LOGIN_COSIGN_V3) ) )
	{
	...
	elseif ( cosign_real_login_method() == SSO_LOGIN_LDAP )
	{
...
function cosign_sso_options_page()
{
	...
	$optionarray_def['login_method'] = cosign_real_login_method();

	// Setup Feed Key Reset Options
	$cosign_login_types = array(
		__('Disabled', 'cosign_sso') => SSO_LOGIN_DISABLED,
		__('LDAP', 'cosign_sso') => SSO_LOGIN_LDAP,
		__('CoSign V3', 'cosign_sso') => SSO_LOGIN_COSIGN_V3,
		__('CoSign V2', 'cosign_sso') => SSO_LOGIN_COSIGN_V2,
	);
...
?>
	<div class="wrap">
	<h2><?php _e("Single SignOn Options", "cosign_sso"); ?></h2>
	...
	print __("Checking the <em>CoSign SSO</em> option below, it will change login method and will affect all users including yourself.<br />".
	         "<strong>Bad configuration will ban all users!!!</strong><br />".
				...
				"simply create a file named <em>\"FALLBACK\"</em> (blank file or contains \"LDAP\", \"COSIGN2\", \"COSIGN3\", ...) ".
				"under this plugin directory, or modify the database record by hands.",
				"cosign_sso");

	if($cosign_sso_fallback == SSO_LOGIN_DISABLED)
		printf("<div><strong>". __("CoSign SSO login disable by file '%s'. To enable CoSign SSO login, remove that file.", "cosign_sso") . "</strong></div>", COSIGN_SSO_FALLBACK_FILE);
	elseif($cosign_sso_fallback == SSO_LOGIN_LDAP)
		printf("<div><strong>". __("Fallback to use LDAP auth mode. To enable CoSign SSO login, remove the file '%s'.", "cosign_sso") . "</strong></div>", COSIGN_SSO_FALLBACK_FILE);
	elseif($cosign_sso_fallback == SSO_LOGIN_COSIGN_V2)
		printf("<div><strong>". __("Fallback to use CoSign V2 auth mode. If this works, set login method to it and remove the file '%s'.", "cosign_sso") . "</strong></div>", COSIGN_SSO_FALLBACK_FILE);
	elseif($cosign_sso_fallback == SSO_LOGIN_COSIGN_V3)
		printf("<div><strong>". __("Fallback to use CoSign V3 auth mode. If this works, set login method to it and remove the file '%s'.", "cosign_sso") . "</strong></div>", COSIGN_SSO_FALLBACK_FILE);
		...
		<tr valign="top">
			...
			<td>
				...
				<?php
					...
					printf("<div><strong>". __("Notes: CoSign 2.x and 3.x are totally different in the core, and not compatible. If you choose wrong CoSign version, you can not login any more.", "cosign_sso") . "</strong></div>");
				?>
				</span>
			</td>
		</tr>
...
if ( cosign_real_login_method() > SSO_LOGIN_DISABLED )
{
	// Common hooks
